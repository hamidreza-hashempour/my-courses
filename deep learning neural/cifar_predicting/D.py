...
model_gradient_descent = tf.keras.models.Sequential([
  #tf.keras.layers.Flatten(),
  tf.keras.layers.Conv2D(filters=80, kernel_size=6, strides=1, padding='same', activation='relu', input_shape=(32, 32, 3)),
  tf.keras.layers.MaxPooling2D(pool_size=2),
  tf.keras.layers.Conv2D(filters=38, kernel_size=4, strides=1, padding='same', activation='relu'),
...
test_acc_ADAM = []
test_acc_ADAM = np.asarray(test_acc_ADAM)
test_acc_gradient_descent = []
test_acc_gradient_descent = np.asarray(test_acc_gradient_descent)
...
for i in range(epok):
  history = model_adam.fit(x_train, y_train, batch_size=200, epochs=1)
  score = model_adam.evaluate(x_test, y_test)
  test_acc_ADAM = np.append(test_acc_ADAM, [score[1]])

for i in range(epok):
  history = model_gradient_descent.fit(x_train, y_train, batch_size=200, epochs=1)
  score = model_gradient_descent.evaluate(x_test, y_test)
  test_acc_gradient_descent = np.append(test_acc_gradient_descent, [score[1]])

plt.plot(test_acc_ADAM[0:epok])
plt.plot(test_acc_gradient_descent[0:epok])
plt.title('model accuracy')
plt.ylabel('accuracy')
plt.xlabel('epoch')
plt.legend(['test_acc_ADAM', 'test_acc_gradient_descent'], loc='upper left')
plt.show()